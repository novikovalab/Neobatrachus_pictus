...
proj_dir   <- "~/Neobatrachus_project"        # base project directory
vcf_file   <- file.path(proj_dir, "vcf", "Neobatrachus.vcf.gz")
func_file  <- file.path(proj_dir, "scripts", "adegenet_functions.r")
out_nexus  <- file.path(proj_dir, "results", "Neobatrachus.ind.nex")
## --------------------------------------

# Custom helper functions for adegenet / genlight
source(func_file)

##########################
# Load VCF and build genlight object
##########################

alp.snps <- read.vcfR(vcf_file)
alp      <- vcfR2genlight.tetra(alp.snps)  # custom function from adegenet_functions.r

##########################
# Assign populations based on sample names
##########################

pop_keywords <- c(
  "aquilonius 4", "sutor", "fulvus", "albipes",
  "pelobatoides", "pictus", "kunapalari 4",
  "wilsmorei", "sudellae 4"
)

pop(alp) <- sapply(indNames(alp), function(x) {
  match <- grep(paste(pop_keywords, collapse = "|"), x, value = TRUE)
  if (length(match) > 0) {
    # return the keyword(s) matching this sample name
    pop_keywords[sapply(pop_keywords, function(kw) grepl(kw, x))]
  } else {
    NA
  }
})

# Quick check: number of individuals per population
table(pop(alp))

##########################
# Filter loci by missing data
##########################

# Proportion of missing genotypes per locus
missing_per_locus <- colMeans(is.na(as.matrix(alp)))

# Keep loci with missing rate below threshold (e.g. < 50%)
threshold <- 0.5
alp       <- alp[, missing_per_locus < threshold]

##########################
# PCA on filtered data
##########################

pca_result <- glPcaFast(alp, center = TRUE, scale = FALSE, nf = 3)

# You can inspect the population sizes again if needed:
# table(pop(alp))

##########################
# Nei's distance between individuals
##########################

# Collapse all samples into a single population "ALL"
pop(alp) <- rep("ALL", nInd(alp))

# Compute Nei's distance among individuals
alp.D.ind <- stamppNeisD(alp, pop = FALSE)

# Write distance matrix to NEXUS format for SplitsTree
write.nexus.dist(alp.D.ind, file = out_nexus)

# Now open the NEXUS matrix in SplitsTree:
# File > Open > select the Neobatrachus.ind.nex file
